...
class Main( ): 
    ...
    card_pass_code = RegexValidator(regex=r'^\+?1?\d{6}$', message="Numeric Field. Only 4 digits allowed.")
...
class BankCard(models.Model):
    ...
    card_amount         = models.PositiveIntegerField(default=20000, verbose_name='account on card')
...
class Record(models.Model):
    ...
    amount_recevied     = models.PositiveIntegerField(blank=True, null=True, validators=[MaxValueValidator(5000)])
    recevier_name       = models.CharField(max_length=30)
    transfer_amount     = models.PositiveIntegerField(blank=True, null=True, validators=[MaxValueValidator(1000000),MinValueValidator(0)])
    ...
    def get_absolute_url(self):
        return reverse('delete-transactions', kwargs={'pk': self.pk})
...
class CardPassCode(models.Model):
    ...
    pass_code               = models.CharField(validators=[Main.card_pass_code], max_length=6)
...
class VerifiedPassCode(models.Model):
    user                    = models.ForeignKey(User, blank=True, null=True, db_constraint=False, on_delete=models.CASCADE)
    verified_pass_code      = models.CharField(validators=[Main.card_pass_code], max_length=6)
